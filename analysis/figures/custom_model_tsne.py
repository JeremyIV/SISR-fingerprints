...
parser.add_argument("--reduction", default="TSNE", help="Either TSNE or UMAP.")
...
if args.reduction == "TSNE":
    ...
    raise NotImplementedError()  # TODO: umap!
else:
...
for row, loss in enumerate(["Adv", "L1"]):
    for col, scale in enumerate([2, 4]):
        ax = axs[row, col]
        if loss == "Adv" and scale == 4:
            ax.set_title("4X")
        if loss == "Adv" and scale == 2:
            ax.set_title("2X")
        if loss == "Adv" and scale == 4:
            ax.set_ylabel("Adv.")
        if loss == "L1" and scale == 4:
            ax.set_ylabel("L₁")
